...
// Added By Sufi for Account page tabs

document.addEventListener('DOMContentLoaded', function() {
  // Tab functionality
  const tabLinks = document.querySelectorAll('.nav-link[data-tab]');
  const tabPanes = document.querySelectorAll('.tab-pane');
  const linkToTabs = document.querySelectorAll('.link-to-tab');
  ...
  // Add click event listeners to in-content tab links
...
// Script For Checkout Page Accordian
document.addEventListener('DOMContentLoaded', function() {
  // Get all accordion triggers
  const accordionTriggers = document.querySelectorAll('a[href^="#alert-body"]');
  ...
  accordionTriggers.forEach(trigger => {
    trigger.addEventListener('click', function(e) {
      ...
      // Get the target body ID from the href attribute
      const targetId = this.getAttribute('href').substring(1);
      const targetBody = document.getElementById(targetId);
      
      // Toggle the hidden class on the target body only
      if (targetBody) {
        targetBody.classList.toggle('hidden');
        
        // Make sure the collapsed class is also toggled
        targetBody.classList.toggle('collapsed');
      }
      
      // Ensure the parent header div remains visible
      const headerDiv = this.closest('.card-header, .alert');
      if (headerDiv) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  radioButtons.forEach(radio => {
    radio.addEventListener('change', function() {
      ...
      const collapseId = this.id === 'payment1' ? 'collapse1' : 'collapse2';
      document.getElementById(collapseId).classList.remove('hidden');
    });
  });
  
  // Initialize - ensure the checked radio button's content is visible
  const checkedRadio = document.querySelector('input[name="payment-method"]:checked');
  if (checkedRadio) {
    const collapseId = checkedRadio.id === 'payment1' ? 'collapse1' : 'collapse2';
    document.getElementById(collapseId).classList.remove('hidden');
  }
});
